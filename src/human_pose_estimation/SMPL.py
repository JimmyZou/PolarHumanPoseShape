...
def batch_global_rigid_transformation(Rs, Js, parent, device):
    ...
    def make_A(R, t):
        ...
        t_homo = torch.cat(
            [t, torch.ones((N, 1, 1), device=device)], dim=1
        )  # [N, 4, 1]
    ...
    Js_w0 = torch.cat(
        [Js, torch.zeros((N, 24, 1, 1), device=device)], dim=2
    )  # [N, 24, 4, 1]
...
class SMPL(nn.Module):
    def __init__(self, model_path, max_batch_size):
        ...
        self.register_buffer(
            "shapedirs", torch.from_numpy(np_shapedirs).float()
        )  # [10, 20670]

        np_J_regressor = np.array(model["J_regressor"].todense().T, dtype=np.float)
        self.register_buffer(
            "J_regressor", torch.from_numpy(np_J_regressor).float()
        )  # [6890, 24]
        ...
        self.register_buffer(
            "posedirs", torch.from_numpy(np_posedirs).float()
        )  # [207, 20670]
        ...
        self.register_buffer(
            "weight",
            torch.from_numpy(np_weights)
            .float()
            .reshape(-1, vertex_count, vertex_component),
        )
    ...
    def save_obj(self, verts, obj_mesh_name):
        vertex = np.array(
            [tuple(i) for i in verts], dtype=[("x", "f4"), ("y", "f4"), ("z", "f4")]
        )
        face = np.array(
            [(tuple(i), 255, 255, 255) for i in self.faces],
            dtype=[
                ("vertex_indices", "i4", (3,)),
                ("red", "u1"),
                ("green", "u1"),
    ...
    def get_root_rt(self, beta, theta):
        ...
        # [N, 6890, 3]
        v_shaped = (
            torch.matmul(beta, self.shapedirs).view(-1, self.size[0], self.size[1])
            + self.v_template
        )
    ...
    def forward(self, beta, theta=None, get_skin=False, rotmats=None):
        ...
        v_posed_homo = torch.cat(
            [
                v_posed,
                torch.ones(num_batch, v_posed.shape[1], 1, device=self.cur_device),
            ],
            dim=2,
        )
...
if __name__ == "__main__":
    model_dir = (
        "/home/shihao/event_project/smpl_model/basicModel_m_lbs_10_207_0_v1.0.0.pkl"
    )

    # device = torch.device('cuda', 1)
    device = torch.device("cpu")
    smpl = SMPL(model_dir, 3).to(device)
